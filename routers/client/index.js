...
router.get('/', (req, res) => {
    // Orijinal repoda bu route "artık desteklenmiyor, v5'e geçin" reklamına
    // sabitlenmişti. Gerçek akışı geri getiriyoruz: giriş yapmış kullanıcıyı
    // panele, yapmamışı login ekranına yönlendir.
    const query = req.query.success !== undefined
        ? `?success=${req.query.success}&message=${encodeURIComponent(req.query.message || '')}`
        : '';
    if (req.cookies.username && req.cookies.pass) {
        return res.redirect(`${sitePages.newOrder.url}${query}`);
    }
...
router.get(sitePages.services.url, checkAuth, async (req, res) => {
    res.render('client/pages/services.ejs', {
        page: sitePages.services,
        platforms: await platformModel.find({ visible: true }).sort({ queue: 1 }),
    });
});
